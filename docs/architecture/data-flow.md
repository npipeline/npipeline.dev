...
var sourcePipe = await sourceNode.CreateDataPipe(context, cancellationToken);
...
var pipe = await source.CreateDataPipe(context, cancellationToken);
...
```csharp
var source = await sourceNode.CreateDataPipe(context, ct);      // IDataPipe<Order>
